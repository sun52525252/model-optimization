...
def quantize_apply(model):
  ...
  def _quantize(layer):  # pylint: disable=missing-docstring
    ...
    if not quantize_config:
      error_msg = (
          'Layer {}:{} is not supported out of the box. You can quantize this '
          'layer by passing a `tfmot.quantization.keras.QuantizeConfig` '
          'instance to the `tfmot.quantization.keras.quantize_annotate_layer` '
          'API. For built-in tf.keras layers, see if there is a Github feature'
          ' request to add it.')
      raise RuntimeError(
