...
Conv2DBatchNormActivationQuantize = default_transforms.Conv2DBatchNormActivationQuantize
Conv2DBatchNormReLUQuantize = default_transforms.Conv2DBatchNormReLUQuantize
...
class DefaultTransformsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def testTransformsConvBNReLUPattern(self):
    ...
    transformed_model, _ = ModelTransformer(
        model, [default_transforms.Conv2DBatchNormReLU6Fold()]).transform()
  ...
  def testTransformsConvBNPattern(self):
    ...
    with quantize.quantize_scope():
      transformed_model, _ = ModelTransformer(
          model, [default_transforms.Conv2DBatchNormFold()]).transform()
  ...
  def testTransformsDepthwiseConvBNReLUPattern(self):
    ...
    transformed_model, _ = ModelTransformer(
        model,
        [default_transforms.DepthwiseConv2DBatchNormReLU6Fold()]).transform()
  ...
  def testConv2DBatchNormQuantize(self, layer_type):
    ...
    self.assertIsInstance(
        ...
        default_quantize_configs.OutputQuantizeConfig)
  ...
  def testConv2DBatchNormReLUQuantize(
    ...
    self.assertIsInstance(
        updated_metadata.get(bn_layer.name).get('quantize_config'),
        default_quantize_configs.NoOpQuantizeConfig)
  ...
  def testAddsQuantizeLayerAfterInputLayer(self):
    ...
    transformed_model, _ = ModelTransformer(
        model, [default_transforms.InputLayerQuantize()]).transform()
  ...
  def testConcatTransform(self):
    ...
    layer_metadata = {
        ...
        dense_1.name: {
            'quantize_config': default_quantize_configs.OutputQuantizeConfig()
        }
    }
    _, updated_metadata = ModelTransformer(
        model, [default_transforms.ConcatTransform()],
    ...
    self.assertIsInstance(concat_quantize_config,
                          default_quantize_configs.OutputQuantizeConfig)
    ...
    self.assertIsInstance(dense_1_quantize_config,
                          default_quantize_configs.OutputQuantizeConfig)
    ...
    self.assertEqual('DefaultQuantizeConfig',
  ...
  def testConcatMultipleLevels(self):
    ...
    _, layer_metadata = ModelTransformer(
        model, [default_transforms.ConcatTransform()]).transform()
